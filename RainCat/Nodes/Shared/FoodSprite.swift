...
public class FoodSprite : SKSpriteNode, Palettable {
  ...
  public static let foodName = ["apple", "banana", "tomato", "mango", "pineapple", "lemon", "kiwi", "pear", "strawberry", "orange"]
    public static var foodIndex = 0;
    
  public static func newInstance(palette : ColorPalette) -> FoodSprite {
    foodIndex = Int(arc4random_uniform(UInt32(foodName.count))); // random index cua mang trai cay
    let foodDish = FoodSprite(imageNamed: foodName[foodIndex]); // set hinh hanh cho foodSprite
